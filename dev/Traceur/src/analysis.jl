...
function get_slot_type(code::Core.CodeInfo, x::Core.SlotNumber)
    slot_index = x.id
    return code.slottypes[slot_index]
end
exprtype(code, x) = typeof(x)
exprtype(code::Core.CodeInfo, x::Core.SlotNumber) = code.slottypes[x.id]
